...
@JsonSerializable(
    explicitToJson: true) // Enable explicitToJson for nested Player list
class AIClub {
  final String id;
  String name;
  int reputation;
  ...
  }) : this.activeTournamentIds =
            activeTournamentIds ?? []; // Initialize with empty list if null
  ...
  factory AIClub.initial(int index, {int initialTier = 3}) {
    ...
    int baseReputation = 150 +
        (3 - initialTier) * 50 +
        random.nextInt(51); // Tier 1: 150-200, Tier 2: 100-150, Tier 3: 50-100
    double initialBalance = 100000.0 +
        (3 - initialTier) * 200000.0 +
        random.nextDouble() *
            100000.0; // Tier 1: 300k-400k, Tier 2: 200k-300k, Tier 3: 100k-200k
    int initialSkill = 30 +
        (3 - initialTier) * 20 +
        random.nextInt(21); // Tier 1: 70-90, Tier 2: 50-70, Tier 3: 30-50
    int initialFanCount = 1000 +
        (3 - initialTier) * 5000 +
        random.nextInt(2001); // Tier 1: 11k-13k, Tier 2: 6k-8k, Tier 3: 1k-3k
    double initialTicketPrice = 10.0 +
        (3 - initialTier) * 15.0 +
        random.nextDouble() *
            5.0; // Tier 1: 40-45, Tier 2: 25-30, Tier 3: 10-15
  ...
  static String _generateClubName(int index, Random random) {
    ...
    if (prefix == location ||
        prefix == suffix ||
        (location == suffix && suffix.isNotEmpty)) {
    ...
    if (prefix == "FC" ||
        ...
        prefix == "Sporting" ||
        prefix == "Olympic") {
      // Prefixes that usually come after
    ...
    if (name.split(" ").length < 2) {
      ...
      if (backupSuffix.isNotEmpty && backupSuffix != location) {
        ...
        name =
            "${prefixes[random.nextInt(prefixes.length)]} $location"; // Add a random prefix
      }
    }

    return name
        .replaceAll("  ", " ")
        .trim(); // Clean up potential double spaces
  }
  ...
  void updateSkillLevel() {
    ...
    double totalSkill =
        players.fold(0, (sum, player) => sum + player.currentSkill);

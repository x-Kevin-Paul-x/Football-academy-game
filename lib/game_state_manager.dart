...
class GameStateManager with ChangeNotifier {
  ...
  Future<bool> loadGame() async {
    try {
      ...
      // --- MIGRATION: Ensure News Items are Newest-First ---
      if (_newsItems.length > 1) {
         // Check if stored in old format (Oldest First)
         // In old format, index 0 is oldest. index N is newest.
         // If first item date is BEFORE last item date, it's likely Oldest First.
         if (_newsItems.first.date.isBefore(_newsItems.last.date)) {
             print("--- Migrating News Items from Oldest-First to Newest-First order ---");

...
import 'dart:io'
    if (dart.library.html) 'src/web_io_stub.dart'; // Use dart:io stub for web
import 'package:path_provider/path_provider.dart'
    if (dart.library.html) 'src/web_path_provider_stub.dart'; // Use path_provider stub for web
...
class MatchTeamSelection {
  ...
  MatchTeamSelection(
      {required this.formation, required this.starters, required this.bench});
}
// --- End Helper Class ---
...
class GameStateManager with ChangeNotifier {
  ...
  static const String playerAcademyId =
      'player_academy_1'; // Unique ID for the player's academy
  ...
  List<Tournament> _activeTournaments =
      []; // Tournaments currently in progress (Scheduled or InProgress)
  List<Tournament> _completedTournaments = []; // Tournaments that have finished
  List<Tournament> _availableTournamentTemplates =
      []; // Templates for new tournaments
  ...
  static const double _baseMerchUnitCost =
      5.0; // Assumed base cost for a "unit" of merchandise
  ...
  // --- Test Helpers ---
  @visibleForTesting
  void addTestNewsItem(NewsItem item) {
    _addNewsItem(item);
  }
  ...
  List<Tournament> get activeTournaments =>
      UnmodifiableListView(_activeTournaments);
  List<Tournament> get completedTournaments =>
      UnmodifiableListView(_completedTournaments);
  List<Tournament> get availableTournamentTemplates =>
      UnmodifiableListView(_availableTournamentTemplates);
  // Rival Academy Getters
  List<RivalAcademy> get rivalAcademies =>
      UnmodifiableListView(_rivalAcademies);
  Map<String, RivalAcademy> get rivalAcademyMap =>
      UnmodifiableMapView(_rivalAcademyMap);
  // AI Club Getters
  List<AIClub> get aiClubs => UnmodifiableListView(_aiClubs);
  Map<String, AIClub> get aiClubMap =>
      UnmodifiableMapView(_aiClubMap); // <-- ADDED GETTER
  // Facility Getters
  ...
  List<Map<String, dynamic>> get transferOffers =>
      UnmodifiableListView(_transferOffers);
  ...
  void _generateInitialAvailableStaff() {
    _availableStaff = List<Staff>.generate(5, (index) {
      // Added <Staff> type argument
      StaffRole role =
          StaffRole.values[_random.nextInt(StaffRole.values.length)];
      ...
      return Staff.randomStaff(
          'staff_${DateTime.now().millisecondsSinceEpoch}_$index', role);
    });
  }
  ...
  void _populateRivalAcademyMap() {
    ...
    _rivalAcademies = List<RivalAcademy>.generate(
        numberOfRivals,
        (index) => RivalAcademy.initial(index,
            difficulty: _difficulty)); // Added <RivalAcademy>
    ...
    for (var academy in _rivalAcademies) {
      ...
      for (int i = 0; i < initialPlayerCount; i++) {
        ...
        int potentialSkill =
            (basePotential + academySkillBonus).clamp(25, 95); // Clamped 25-95

        // Current skill as a percentage of this new potential.
        // Rivals start with players at 40-80% of their potential.
        int currentSkill =
            (potentialSkill * (0.4 + _random.nextDouble() * 0.4)).toInt();
        currentSkill =
            currentSkill.clamp(15, potentialSkill); // Clamp current skill
        ...
        Player newPlayer = Player.createWithTargetSkill(
          ...
          naturalPosition: PlayerPosition
              .values[_random.nextInt(PlayerPosition.values.length)],
    ...
    print(
        "Populated Rival Academy Map with ${_rivalAcademyMap.length} academies.");
  }
  ...
  void _populateAIClubs() {
    ...
    for (int i = 0; i < totalAIClubs; i++) {
      int tier =
          (i ~/ clubsPerTier) + 1; // Assign tier based on index (1, 2, 3)
    ...
    for (var club in _aiClubs) {
      ...
      // --- END INCREASE ---
      for (int i = 0; i < initialPlayerCount; i++) {
        int potentialSkill = (club.skillLevel * 1.4).toInt() +
            _random.nextInt(21); // Higher base potential
        potentialSkill = potentialSkill.clamp(1, 99); // Clamp potential
        int currentSkill = (potentialSkill * (0.6 + _random.nextDouble() * 0.3))
            .toInt(); // 60-90% of potential
        currentSkill =
            currentSkill.clamp(10, potentialSkill); // Clamp current skill
        PlayerPosition position = PlayerPosition.values[
            _random.nextInt(PlayerPosition.values.length)]; // Define position

        // Use the new factory method for AI Clubs as well
        club.players.add(Player.createWithTargetSkill(
  ...
  void _generateInitialTournamentTemplates() {
    ...
    print(
        "Generated ${_availableTournamentTemplates.length} tournament templates.");
  }
  ...
  void _applyDifficultySettings() {
    ...
    switch (_difficulty) {
      case Difficulty.Easy:
        _academyReputation = 30;
        break;
      case Difficulty.Normal:
        _academyReputation = 10;
        break;
      case Difficulty.Hard:
        _academyReputation = 5;
        break;
      case Difficulty.Hardcore:
        _academyReputation = 0;
        break;
    }
    print(
        "Applied difficulty settings for $_difficulty. Rep=$_academyReputation");
  }
  ...
  void resetGame() {
    ...
    print(
        "--- DEBUG (resetGame): _transferOffers cleared. Length: ${_transferOffers.length} ---");
    _newsItems.clear();
  ...
  void advanceWeek() {
    ...
    if (_isForcedSellActive && _financeService.balance < 0) {
      ...
      _addNewsItem(NewsItem.create(
          ...
          description:
              "You must sell players to raise funds before continuing.",
    ...
    BankruptcyStatus status =
        _financeService.checkBankruptcyStatus(_difficulty);
    ...
    if (status == BankruptcyStatus.GameOver) {
      ...
      _addNewsItem(NewsItem.create(
          ...
          description:
              "The academy has run out of funds and credit. The board has dissolved the club.",
          type: NewsItemType.Finance,
          date: _timeService.currentDate));
      notifyListeners();
      return;
    ...
    if (status == BankruptcyStatus.ForcedSell) {
      ...
      _addNewsItem(NewsItem.create(
          ...
          description:
              "We are in debt. The board mandates immediate player sales to balance the books. You cannot continue until you are in the green.",
          type: NewsItemType.Finance,
          date: _timeService.currentDate));
    } else {
    ...
    if (status == BankruptcyStatus.Warning) {
      _addNewsItem(NewsItem.create(
          ...
          description:
              "We are operating at a loss. Continued debt will lead to bankruptcy.",
          type: NewsItemType.Finance,
          date: _timeService.currentDate));
    }
    ...
    final activeScouts = _hiredStaff
        .where((s) => s.role == StaffRole.Scout && s.isAssigned)
        .toList();
    int playersFoundThisWeekByAllScouts = 0;
    const int maxScoutedPlayersCap =
        20; // Max players to keep in the scouted list
    ...
    if (activeScouts.isNotEmpty) {
      for (var scout in activeScouts) {
        ...
        double facilityBonus = _scoutingFacilityLevel /
            20.0; // Max 0.25 for level 5 (assuming max level 5 for now)
        double findChance = (baseFindChance + scoutSkillBonus + facilityBonus)
            .clamp(0.1, 0.9); // Clamp chance
        ...
        if (_random.nextDouble() < findChance) {
          ...
          if (scout.skill > 70 &&
              _scoutingFacilityLevel > 2 &&
              _random.nextDouble() < 0.3) {
            numPlayersFoundByScout++;
          }
          if (scout.skill > 85 &&
              _scoutingFacilityLevel > 3 &&
          ...
          for (int i = 0; i < numPlayersFoundByScout; i++) {
            ...
            if (_scoutedPlayers.length < (maxScoutedPlayersCap + 10)) {
              // Allow a buffer before sorting and final trim
              final newPlayerId =
                  'scouted_${_timeService.currentDate.millisecondsSinceEpoch}_${scout.id}_$i';
              // Use the scout's skill to influence player generation
              Player newPlayer = Player.randomScoutedPlayer(newPlayerId,
                  scoutSkill: scout.skill);
      ...
      if (_scoutedPlayers.isNotEmpty) {
        ...
        if (_scoutedPlayers.length > maxScoutedPlayersCap) {
          _scoutedPlayers.removeRange(
              maxScoutedPlayersCap, _scoutedPlayers.length);
        }
      }
    }
    ...
    if (playersFoundThisWeekByAllScouts > 0) {
      _addNewsItem(NewsItem.create(
          ...
          description:
              "Our scouts have identified $playersFoundThisWeekByAllScouts potential new talent${playersFoundThisWeekByAllScouts > 1 ? 's' : ''} this week.",
          type: NewsItemType.Scouting,
          date: _timeService.currentDate));
    } else if (activeScouts.isNotEmpty) {
      // Only report "no players found" if scouts were active
      _addNewsItem(NewsItem.create(
          title: "Scouting Report",
          description:
              "Scouts found no notable players this week despite their efforts.",
          type: NewsItemType.Scouting,
          date: _timeService.currentDate));
    }
    ...
    _addNewsItem(NewsItem.create(
        ...
        description:
            "Income: ${currencyFormat.format(_financeService.weeklyIncome)}, Wages: ${currencyFormat.format(_financeService.totalWeeklyWages)}. Net: ${currencyFormat.format(netChange)}. Balance: ${currencyFormat.format(_financeService.balance)}",
        type: NewsItemType.Finance,
        date: _timeService.currentDate));
  ...
  void _checkForNewTournaments() {
    ...
    bool forcedAttempt =
        false; // Flag to ensure at least one attempt if random fails

    // Shuffle templates to vary which ones are considered first
    List<Tournament> shuffledTemplates =
        List.from(_availableTournamentTemplates)..shuffle(_random);
    ...
    for (var template in shuffledTemplates) {
      ...
      bool alreadyActive = _activeTournaments.any((t) =>
      ...
      double baseChance = (template.format == TournamentFormat.League)
          ? 0.50
          : 0.80; // Lower chance for league (Increased from 0.25 / 0.50)
      double repModifier = (template.requiredReputation / 500.0)
          .clamp(0.1, 0.5); // Rep still has some influence (10-50%)
      double startChance = baseChance +
          (_random.nextDouble() * repModifier); // Base + Random Rep Influence
      ...
      if (tournamentsScheduledThisMonth == 0 && !forcedAttempt) {
        ...
        if (template == shuffledTemplates.last) {
          print(
              " -> Forcing attempt for ${template.name} as last chance this month.");
        ...
        // Or, give a much higher chance earlier on if nothing scheduled yet
          ...
          // 80% chance to force an earlier attempt
          print(
              " -> Increasing chance for ${template.name} as none scheduled yet.");
      ...
      if (_random.nextDouble() < startChance) {
        print(
            " -> Considering scheduling ${template.name} (Chance: ${startChance.toStringAsFixed(2)})...");
        ...
        if (template.isAIClubFocusedLeague) {
          ...
          if (leagueTier == 1 || leagueTier == 2) {
            ...
            eligibleAIClubs = _aiClubs
                .where((club) =>
                    ...
                    !club.activeTournamentIds.any(
                        (tId) => _activeTournaments.any((at) => at.id == tId)))
                .toList();
            print(
                " -> Tier $leagueTier League: Filtering for clubs currently in Tier $leagueTier.");
          } else {
            // For Tier 3 (or if tier unknown), use reputation as the primary filter (entry point)
            eligibleAIClubs = _aiClubs
                .where((club) =>
                    (club.tier == 3 ||
                        club.tier == 0) && // Allow Tier 3 or unassigned tier
                ...
                .toList();
            print(
                " -> Tier 3 League: Filtering for Tier 3/unassigned clubs meeting reputation req.");
          }

          eligibleAIClubs.sort((a, b) =>
              b.reputation.compareTo(a.reputation)); // Sort by reputation desc
          ...
          int youthRepReq = template.youthAcademyMinReputation ??
              template
                  .requiredReputation; // Use specific youth req if available

          if (leagueTier == 3 || (leagueTier == 2 && youthRepReq >= 400)) {
            // Allow high-rep youth in T2
            // Rivals
            ...
            playerEligible = _academyReputation >= youthRepReq &&
                ...
                !_activeTournaments
                    .any((at) => at.teamIds.contains(playerAcademyId));

            eligibleYouthAcademies.sort((a, b) => b.reputation
                .compareTo(a.reputation)); // Sort by reputation desc
            print(
                " -> Found ${eligibleAIClubs.length} eligible AI Clubs and ${eligibleYouthAcademies.length} eligible Youth Academies (Player eligible: $playerEligible) for Tier $leagueTier League.");
            ...
            print(
                " -> Tier $leagueTier League: Youth academies not eligible based on current rules.");
            // eligibleYouthAcademies remains empty, playerEligible remains false
          }
          ...
          for (var academy in eligibleYouthAcademies) {
            ...
            if (academy is RivalAcademy &&
                academy.shouldEnterTournament(
                    template,
                    _timeService.currentDate.year,
                    _timeService.currentDate.month)) {
              participants.add(academy.id);
            }
            // Player doesn't get added here, only eligibility is checked
          }
          print(
              " -> Added ${participants.length - aiClubsAdded} Youth Academies.");

          // 4. Check if minimum met (considering potential player join)
          int minRequired = template.minTeamsToStart;
          if (playerEligible)
            minRequired--; // Player can potentially fill one slot

          if (participants.length >= minRequired) {
            enoughParticipantsFound = true;
          } else {
            print(
                " -> Not enough participants (${participants.length}) for AI Focused League ${template.name}. Min required (allowing for player): $minRequired");
          }
        }
        ...
        // --- ELSE: Standard Tournament Logic (Rivals Only Initially) ---
          ...
          List<RivalAcademy> potentialRivals = _rivalAcademies
              .where((rival) =>
                      rival.reputation >= template.requiredReputation &&
                      rival.balance >= template.entryFee &&
                      rival.players.length >= template.requiredPlayers &&
                      !rival.activeTournamentIds.any((tId) =>
                          _activeTournaments.any((at) =>
                              at.id == tId)) // Not already in an active one
                  )
              .toList();
          ...
          for (var rival in potentialRivals) {
            ...
            if (rival.shouldEnterTournament(
                template,
                _timeService.currentDate.year,
                _timeService.currentDate.month)) {
          ...
          bool playerEligible =
              _academyReputation >= template.requiredReputation &&
                  _financeService.canAfford(template.entryFee.toDouble()) &&
                  _academyPlayers.length >= template.requiredPlayers &&
                  !_activeTournaments
                      .any((at) => at.teamIds.contains(playerAcademyId));
          ...
          if (participants.length >= minRequired) {
            ...
            print(
                " -> Not enough potential participants (${participants.length}) for Standard Tournament ${template.name}. Min required (allowing for player): $minRequired");
          }
        }
        ...
        if (enoughParticipantsFound) {
          Tournament newTournament = Tournament.fromTemplate(
              template, participants, _timeService.currentDate);
          addActiveTournament(
              newTournament); // Adds to _activeTournaments with Scheduled status

          // Mark participants as active in this tournament
          for (var participantId in participants) {
            if (_rivalAcademyMap.containsKey(participantId)) {
              _rivalAcademyMap[participantId]
                  ?.activeTournamentIds
                  .add(newTournament.id);
            } else if (_aiClubMap.containsKey(participantId)) {
              _aiClubMap[participantId]
                  ?.activeTournamentIds
                  .add(newTournament.id);
            }
          }
          ...
          _addNewsItem(NewsItem.create(
              ...
              description:
                  "The ${newTournament.name} is scheduled to start on ${DateFormat.yMMMd().format(newTournament.startDate)} with ${participants.length} teams confirmed so far. $joinWindow",
              type: NewsItemType.Tournament,
              date: _timeService.currentDate));
          tournamentsScheduledThisMonth++;
          print(
              " -> Scheduled ${newTournament.name} (ID: ${newTournament.id}) starting ${DateFormat.yMMMd().format(newTournament.startDate)} with ${participants.length} teams.");
        }
        // --- END Create Instance ---
      } else {
        print(
            " -> Skipping ${template.name} this month (Rolled < ${startChance.toStringAsFixed(2)} chance).");
      }
    }
  ...
  void _handleRivalAcademyActions() {
    ...
    for (var academy in _rivalAcademies) {
      ...
      if (upgradeType != null) {
        ...
        switch (upgradeType) {
          case 'training':
            currentLevel = academy.trainingFacilityLevel;
            break;
          case 'scouting':
            currentLevel = academy.scoutingFacilityLevel;
            break;
          case 'medical':
            currentLevel = academy.medicalBayLevel;
            break;
        }
        int cost = academy.calculateFacilityUpgradeCost(currentLevel);

        if (academy.balance >= cost) {
          // Check affordability before upgrading
          academy.balance -= cost;
          int newLevel = 0;
          if (upgradeType == 'training') {
            academy.trainingFacilityLevel++;
            newLevel = academy.trainingFacilityLevel;
          } else if (upgradeType == 'scouting') {
            academy.scoutingFacilityLevel++;
            newLevel = academy.scoutingFacilityLevel;
          } else if (upgradeType == 'medical') {
            academy.medicalBayLevel++;
            newLevel = academy.medicalBayLevel;
          }
      ...
      double trainingEffectiveness = (academy.trainingFacilityLevel * 0.5) +
          (academy.skillLevel / 100.0); // Based on facility and skill
      for (var player in academy.players) {
        ...
        if (player.currentSkill < player.potentialSkill) {
          // Check if there's room to improve overall
          double improveChance =
              0.02 * trainingEffectiveness; // Low base chance
        ...
        player.fatigue = (player.fatigue - (5.0 + (player.stamina / 15.0)))
            .clamp(0.0, 100.0);
      }
      ...
      double rivalWeeklyIncome = (academy.reputation * 5) +
          (academy.skillLevel * 10); // Income based on rep/skill
      double rivalWeeklyWages =
          academy.players.fold(0.0, (sum, p) => sum + p.weeklyWage);
  ...
  void _handleAIClubActions() {
    ...
    for (var club in _aiClubs) {
      ...
      double baseWeeklyIncome = (club.reputation * 50) // Increased from 10
          ...
          (club.tier == 1
              ? 20000
              : (club.tier == 2 ? 10000 : 5000)); // Increased tier bonuses
      double aiWeeklyWages =
          club.players.fold(0.0, (sum, p) => sum + p.weeklyWage);
      ...
      double transferActivityChance = 0.10 +
          ((4 - club.tier) * 0.05); // Tier 1: 25%, Tier 2: 20%, Tier 3: 15%
      bool alreadyMadeOfferThisWeekByThisClub = _transferOffers.any((o) =>
          o['offeringClubId'] == club.id &&
          o['dateEpoch'] == _timeService.currentDate.millisecondsSinceEpoch);

      if (!alreadyMadeOfferThisWeekByThisClub &&
          _random.nextDouble() < transferActivityChance) {
        // A. Assess Squad Needs
        Map<String, int> desiredCountsPerGroup = {
          "GK": 2,
          "DEF":
              club.tier == 1 ? 6 : 5, // Adjusted counts for generic positions
          "MID": club.tier == 1 ? 7 : 6, // Adjusted counts
          "FWD": club.tier == 1 ? 3 : 2, // Adjusted counts
        };
        Map<String, PlayerPosition> positionGroups = {
          // Simplified to single PlayerPosition per group
        ...
        List<PlayerPosition> neededPositions =
            []; // Renamed from neededSpecificPositions
        positionGroups.forEach((groupName, positionInGroup) {
          int currentCountInGroup = club.players
              .where((p) => p.naturalPosition == positionInGroup)
              .length;
        ...
        if (neededPositions.isEmpty &&
            club.players.length <
                (club.tier == 1 ? 22 : (club.tier == 2 ? 20 : 18))) {
          neededPositions
              .addAll(PlayerPosition.values); // Consider all generic positions
        }
        ...
        if (neededPositions.isNotEmpty) {
          ...
          for (var player in _academyPlayers) {
            PlayerPosition targetPos =
                player.naturalPosition; // Use natural position directly
            if (neededPositions.contains(targetPos)) {
              // Check against the list of needed generic positions
              if (player.age < 23 &&
                  player.potentialSkill >
                      (club.skillLevel * 0.7 + (3 - club.tier) * 5) &&
                  club.balance > player.calculateMarketValue() * 0.5) {
                // Initial affordability check
                if (!_transferOffers.any((o) =>
                    o['playerId'] == player.id &&
                    o['offeringClubId'] == club.id)) {
                  potentialTargetsData.add({
                    'player': player,
                    'ownerId': playerAcademyId,
                    'ownerName': _academyName,
                    'position': targetPos
                  });
                }
              }
            ...
            for (var player in rivalAcademy.players) {
              ...
              if (neededPositions.contains(targetPos)) {
                ...
                if (player.age < 21 && // Stricter age for rival players
                    player.potentialSkill >
                        (club.skillLevel * 0.65 + (3 - club.tier) * 5) &&
                    club.balance > player.calculateMarketValue() * 0.5) {
                  // Initial affordability check
                  ...
                  bool existingOfferToRival = _transferOffers.any((o) =>
                      ...
                      o['sellingClubId'] ==
                          rivalAcademy
                              .id && // Ensure it's for this specific rival
                      o['dateEpoch'] ==
                          _timeService.currentDate
                              .millisecondsSinceEpoch); // Check for offer made this exact week

                  if (!existingOfferToRival) {
                    // Only add if no recent offer to this rival for this player
                    potentialTargetsData.add({
                      'player': player,
                      'ownerId': rivalAcademy.id,
                      'ownerName': rivalAcademy.name,
                      'position': targetPos
                    });
                  }
                }
              ...
              if (neededPositions.contains(targetPos)) {
                ...
                if (player.age < 28 && // Broader age range
                    player.currentSkill >
                        (club.skillLevel * 0.8 +
                            (3 - club.tier) *
                                3) && // Target slightly better or comparable players
                  ...
                  bool existingOfferToAI = _transferOffers.any((o) =>
                      ...
                      o['sellingClubId'] ==
                          otherAIClub
                              .id && // Ensure it's for this specific AI club
                      o['dateEpoch'] ==
                          _timeService.currentDate.millisecondsSinceEpoch);
          ...
          if (potentialTargetsData.isNotEmpty) {
            potentialTargetsData.sort((a, b) {
              Player pA = a['player'] as Player;
              Player pB = b['player'] as Player;
            ...
            if (club.tier == 1) {
              offerMultiplierMin = 0.85;
              offerMultiplierMax = 1.6;
            } else if (club.tier == 2) {
              offerMultiplierMin = 0.8;
              offerMultiplierMax = 1.45;
            }
            double potentialGapBonus =
                ((targetPlayer.potentialSkill - targetPlayer.currentSkill) /
                        50.0)
                    .clamp(0.0, 0.3);
            offerMultiplierMax += potentialGapBonus;

            double offerMultiplier = offerMultiplierMin +
                _random.nextDouble() *
                    (offerMultiplierMax - offerMultiplierMin);
            ...
            if (club.balance >= offerAmount) {
              ...
              if (targetPlayerOwnerId == playerAcademyId) {
                _addNewsItem(NewsItem.create(
                    ...
                    description:
                        "${club.name} has made an offer of ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(offerAmount)} for your player ${targetPlayer.name}.",
      ...
      double trainingEffectiveness =
          (club.skillLevel / 100.0); // Simple skill-based training
      for (var player in club.players) {
        ...
        if (player.currentSkill < player.potentialSkill) {
          ...
          double improveChance =
              0.002 * trainingEffectiveness; // Was 0.015, significantly reduced
          if (_random.nextDouble() < improveChance) {
      ...
      if (_timeService.currentDate.weekday == DateTime.monday) {
        // Example: Recalculate weekly
  ...
  void _calculateWeeklyWages() {
    int staffWages =
        _hiredStaff.fold(0, (sum, staff) => sum + staff.weeklyWage);
    int playerWages =
        _academyPlayers.fold(0, (sum, player) => sum + player.weeklyWage);
    _financeService.updateWeeklyWages(staffWages + playerWages);
  }

  bool hireStaff(Staff staffToHire) {
    int currentCount =
        _hiredStaff.where((s) => s.role == staffToHire.role).length;
    bool canHire = true;
    String? reason;
    ...
    switch (staffToHire.role) {
      case StaffRole.Manager:
        if (currentCount >= 1) {
          canHire = false;
          reason = "Only one Manager allowed.";
        }
        break;
      case StaffRole.Coach:
        if (currentCount >= _maxCoaches) {
          canHire = false;
          reason =
              "Coach limit reached ($_maxCoaches). Upgrade Training Facility.";
        }
        break;
      case StaffRole.Scout:
        if (currentCount >= _maxScouts) {
          canHire = false;
          reason =
              "Scout limit reached ($_maxScouts). Upgrade Scouting Facility.";
        }
        break;
      case StaffRole.Physio:
        if (currentCount >= _maxPhysios) {
          canHire = false;
          reason = "Physio limit reached ($_maxPhysios). Upgrade Medical Bay.";
        }
        break;
      case StaffRole.MerchandiseManager:
        if (currentCount >= totalMerchManagerCap) {
          canHire = false;
          reason =
              "Merchandise Manager limit reached ($totalMerchManagerCap). Upgrade Merchandise Store for more Store Managers.";
        }
        break;
    }

    if (!canHire) {
      print("Cannot hire ${staffToHire.name}. Reason: $reason");
      return false;
    }
    ...
    _addNewsItem(NewsItem.create(
        title: "Staff Hired",
        description:
            "We have hired ${staffToHire.name} as our new ${staffToHire.role.toString().split('.').last}.",
  ...
  void fireStaff(Staff staffToFire) {
    ...
    if (!_hiredStaff.any((s) => s.id == staffToFire.id)) {
      print(
          "Error: Cannot fire staff ${staffToFire.name} (ID: ${staffToFire.id}). Not found in hired staff.");
      return;
    }

    // If firing a coach, unassign their players first
    if (staffToFire.role == StaffRole.Coach) {
      List<String> playersToUnassign = List.from(staffToFire
          .assignedPlayerIds); // Copy list to avoid modification issues
      for (String playerId in playersToUnassign) {
        unassignPlayerFromCoach(
            playerId, staffToFire.id); // Use existing method
        print(
            " -> Unassigned player ID $playerId from fired coach ${staffToFire.name}");
      }
    }
    ...
    _addNewsItem(NewsItem.create(
        title: "Staff Fired",
        description:
            "We have parted ways with ${staffToFire.name} (${staffToFire.role.toString().split('.').last}).",
        type: NewsItemType.StaffChange,
        date: _timeService.currentDate));
  ...
  void signPlayer(Player playerToSign) {
    ...
    _addNewsItem(NewsItem.create(
        title: "Player Signed",
        description:
            "We have signed the promising young player ${playerToSign.name} to the academy.",
        type: NewsItemType.PlayerSigned,
        date: _timeService.currentDate));
    print("Signed ${playerToSign.name}");

    Staff? availableCoach = _hiredStaff.firstWhereOrNull((s) =>
        s.role == StaffRole.Coach &&
        s.assignedPlayerIds.length < s.maxPlayersTrainable);
    if (availableCoach != null) {
      assignPlayerToCoach(playerToSign.id, availableCoach.id); // Use the method
      print(
          "Automatically assigned ${playerToSign.name} to coach ${availableCoach.name}.");
      _addNewsItem(NewsItem.create(
          title: "Player Assigned",
          description:
              "${playerToSign.name} has been automatically assigned to Coach ${availableCoach.name} for training.",
          type: NewsItemType.Training,
          date: _timeService.currentDate));
    } else {
      print(
          "No coaches with available capacity found for ${playerToSign.name}.");
    }
    notifyListeners();
  }
  ...
  void releasePlayer(Player playerToRelease) {
    ...
    if (!_academyPlayers.any((p) => p.id == playerToRelease.id)) {
      print(
          "Error: Cannot release player ${playerToRelease.name} (ID: ${playerToRelease.id}). Not found in academy.");
      return;
    }
    ...
    _addNewsItem(NewsItem.create(
        ...
        description:
            "We have released ${playerToRelease.name} from the academy.",
        type: NewsItemType
            .PlayerSigned, // Using PlayerSigned for now, maybe add PlayerReleased later?
        date: _timeService.currentDate));
  ...
  bool tryJoinTournament(Tournament template) {
    print(
        "Attempting to join tournament template: ${template.name} (ID: ${template.id})");

    // 1. Check Requirements
    if (_academyPlayers.length < template.requiredPlayers) {
      print(
          " -> Failed: Not enough players (${_academyPlayers.length}/${template.requiredPlayers})");
      return false;
    }
    if (_academyReputation < template.requiredReputation) {
      print(
          " -> Failed: Not enough reputation (${_academyReputation}/${template.requiredReputation})");
      return false;
    }
    if (!_financeService.canAfford(template.entryFee.toDouble())) {
      print(
          " -> Failed: Not enough balance (${_financeService.balance}/${template.entryFee})");
      return false;
    }
    // Check if already joined an instance of this template
    if (_activeTournaments.any((at) =>
        at.baseId == template.id && at.teamIds.contains(playerAcademyId))) {
      print(
          " -> Failed: Already joined an active instance of this tournament.");
      return false;
    }

    // 2. Find a Scheduled Instance of this Template
    Tournament? scheduledInstance = _activeTournaments.firstWhereOrNull((t) =>
        t.baseId == template.id && t.status == TournamentStatus.Scheduled);
    ...
    if (scheduledInstance.teamIds.length >= scheduledInstance.numberOfTeams) {
      print(
          " -> Failed: Scheduled instance ${scheduledInstance.id} is already full (${scheduledInstance.teamIds.length}/${scheduledInstance.numberOfTeams}).");
      return false;
    }

    // 4. Join the Instance
    _financeService
        .deductExpense(template.entryFee.toDouble()); // Deduct entry fee
    scheduledInstance.teamIds
        .add(playerAcademyId); // Add player to participants
    // If league, add player to standings
    if (scheduledInstance.format == TournamentFormat.League) {
      scheduledInstance.leagueStandings[playerAcademyId] =
          LeagueStanding(teamId: playerAcademyId);
    }
    print(
        " -> Success: Joined scheduled tournament instance ${scheduledInstance.id}. Deducted fee: ${template.entryFee}. New balance: ${_financeService.balance}.");
    _addNewsItem(NewsItem.create(
        title: "Tournament Joined",
        description: "Successfully joined the upcoming ${template.name}.",
        type: NewsItemType.Tournament,
        date: _timeService.currentDate));
  ...
  void _simulateMatchesForWeek() {
    DateTime startOfWeek =
        _timeService.currentDate.subtract(const Duration(days: 7));
    DateTime endOfWeek = _timeService.currentDate;
    List<Tournament> completedOrCancelledTournamentsThisWeek = [];
    List<Tournament> tournamentsToCheckForNextRound =
        []; // Knockout tournaments potentially ready for next round

    // --- Handle Tournament Starts ---
    List<Tournament> tournamentsToCheckStart = List.from(_activeTournaments
        .where((t) => t.status == TournamentStatus.Scheduled));
    for (var tournament in tournamentsToCheckStart) {
      ...
      if (!tournament.startDate.isAfter(endOfWeek)) {
        print(
            "Tournament ${tournament.name} (ID: ${tournament.id}) is due to start.");
        ...
        if (tournament.status == TournamentStatus.Cancelled) {
          ...
          print(
              " -> Tournament cancelled - not enough teams (${tournament.teamIds.length}/${tournament.minTeamsToStart}).");
          _addNewsItem(NewsItem.create(
              title: "${tournament.name} Cancelled",
              description:
                  "The ${tournament.name} was cancelled due to insufficient participants.",
              type: NewsItemType.Tournament,
          ...
          for (var rivalId in tournament.teamIds) {
            if (rivalId != playerAcademyId) {
              ...
              _rivalAcademyMap[rivalId]
                  ?.activeTournamentIds
                  .remove(tournament.id);
            }
          }
          completedOrCancelledTournamentsThisWeek
              .add(tournament); // Move cancelled to completed list
        } else if (tournament.status != TournamentStatus.Cancelled) {
          // Check if not cancelled by generateMatchesForStart
          ...
          if (tournament.matches.isNotEmpty) {
            ...
            _addNewsItem(NewsItem.create(
                ...
                description: "The ${tournament.name} has officially begun!",
                type: NewsItemType.Tournament,
                date: tournament.startDate));
          } else {
            // Matches were NOT generated, but it wasn't cancelled either. Log a warning.
            print(
                " -> WARNING: Tournament ${tournament.name} (ID: ${tournament.id}) met start date condition but generateMatchesForStart() resulted in zero matches. Status remains ${tournament.status}. Investigate Tournament.generateMatchesForStart().");
          }
        }
    ...
    for (var tournament in List.from(_activeTournaments)) {
      // Iterate over a copy
      if (tournament.status == TournamentStatus.InProgress) {
        ...
        for (var match in tournament.matches) {
          ...
          if (!match.isSimulated && !match.matchDate.isAfter(endOfWeek)) {
            ...
            if (match.homeTeamId == playerAcademyId) {
              ...
              AIClub? homeAIClub =
                  _aiClubMap[match.homeTeamId]; // Check AI Club Map

              if (homeRival != null) {
                homeSelection =
                    selectRivalTeamForMatch(tournamentType, homeRival);
              } else if (homeAIClub != null) {
                // If AI Club found...
                homeSelection = selectAIClubTeamForMatch(
                    tournamentType, homeAIClub); // Call AI Club selection
              } else {
                // Fallback if team not found in EITHER map
                print(
                    "Error: Home Team ${match.homeTeamId} not found as Player, Rival, or AI Club for match ${match.id}. Using empty lineup."); // <-- NEW ERROR PRINT
                homeSelection = MatchTeamSelection(
                    formation: predefinedFormations.first,
                    starters: [],
                    bench: []);
              }
            }
            ...
            if (match.awayTeamId == playerAcademyId) {
              ...
              print(
                  "DEBUG: _rivalAcademyMap contains key? ${_rivalAcademyMap.containsKey(match.awayTeamId)}");
              print(
                  "DEBUG: _aiClubMap contains key? ${_aiClubMap.containsKey(match.awayTeamId)}");
              // --- DEBUG PRINTS END ---

              // Check if it's a Rival or AI Club
              RivalAcademy? awayRival = _rivalAcademyMap[match.awayTeamId];
              AIClub? awayAIClub =
                  _aiClubMap[match.awayTeamId]; // Check AI Club Map

              if (awayRival != null) {
                awaySelection =
                    selectRivalTeamForMatch(tournamentType, awayRival);
              } else if (awayAIClub != null) {
                // If AI Club found...
                awaySelection = selectAIClubTeamForMatch(
                    tournamentType, awayAIClub); // Call AI Club selection
              } else {
                // Fallback if team not found in EITHER map
                print(
                    "Error: Away Team ${match.awayTeamId} not found as Player, Rival, or AI Club for match ${match.id}. Using empty lineup."); // <-- NEW ERROR PRINT
            ...
            Staff? playerManager = _hiredStaff
                .firstWhereOrNull((s) => s.role == StaffRole.Manager);
            ...
            match.simulateDetailed(
                ...
                isKnockout: tournament.format ==
                    TournamentFormat.Knockout, // Pass knockout status
                playerManager: (match.homeTeamId == playerAcademyId ||
                ...
                tournamentReputation:
                    tournament.requiredReputation // Add tournament reputation
                );

            // --- Update Stats/Fatigue for ALL involved teams ---
            _updatePlayerStatsAndFatigue(match, homeSelection.starters,
                awaySelection.starters); // Player Academy
            _updateRivalOrAIClubFatigue(
                match,
                homeSelection.starters,
                awaySelection
                    .starters); // Rivals AND AI Clubs (Use combined function)
            _updateReputationAfterMatch(
                tournament, match); // Handles Player, Rivals, and AI Clubs
            ...
            if (match.isSimulated &&
                match.result != null &&
                (match.homeTeamId == playerAcademyId ||
                    match.awayTeamId == playerAcademyId)) {
              String homeTeamName = match.homeTeamId == playerAcademyId
                  ? _academyName
                  : (_rivalAcademyMap[match.homeTeamId]?.name ??
                      match.homeTeamId);
              String awayTeamName = match.awayTeamId == playerAcademyId
                  ? _academyName
                  : (_rivalAcademyMap[match.awayTeamId]?.name ??
                      match.awayTeamId);
              ...
              _addNewsItem(NewsItem.create(
                  ...
                  description:
                      "$_academyName $resultString $opponentName $score.",
        ...
        if (matchesSimulatedThisWeek &&
          ...
          int expectedMatchesThisRound = tournament.matches
              .where((Match m) => m.round == tournament.currentRound)
              .length;
          int simulatedMatchesThisRound = tournament.matches
              .where((Match m) =>
                  m.round == tournament.currentRound && m.isSimulated)
              .length;

          if (expectedMatchesThisRound > 0 &&
              simulatedMatchesThisRound == expectedMatchesThisRound) {
            print(
                "Knockout round ${tournament.currentRound} of ${tournament.name} completed.");
            if (!tournamentsToCheckForNextRound.contains(tournament)) {
              tournamentsToCheckForNextRound
                  .add(tournament); // Add to list to process after main loop
            }
          }
        }
      ...
      if (tournament.status == TournamentStatus.InProgress &&
        ...
        bool allMatchesSimulated =
            tournament.matches.every((Match match) => match.isSimulated);
        // --- END FIX ---
        if (allMatchesSimulated) {
          print(
              " -> League ${tournament.name} (ID: ${tournament.id}) has completed all matches.");
          ...
          if (!completedOrCancelledTournamentsThisWeek.contains(tournament)) {
            ...
            for (var participantId in tournament.teamIds) {
              if (participantId != playerAcademyId) {
                ...
                _aiClubMap[participantId]
                    ?.activeTournamentIds
                    .remove(tournament.id); // Also remove from AI clubs
              }
            }
    ...
    for (var tournament in tournamentsToCheckForNextRound) {
      ...
      if (tournament.status == TournamentStatus.InProgress) {
        ...
        if (nextRoundGenerated) {
          print(
              "Generated next knockout round (${tournament.currentRound}) for ${tournament.name}.");
          _addNewsItem(NewsItem.create(
              title: "${tournament.name} Update",
              description:
                  "Round ${tournament.currentRound - 1} completed. Fixtures for Round ${tournament.currentRound} are set.",
              type: NewsItemType.Tournament,
              date: _timeService.currentDate));
        } else {
          ...
          if (tournament.status == TournamentStatus.Completed) {
            print(
                "Tournament ${tournament.name} completed after generating final round.");
            if (!completedOrCancelledTournamentsThisWeek.contains(tournament)) {
              completedOrCancelledTournamentsThisWeek.add(tournament);
    ...
    if (completedOrCancelledTournamentsThisWeek.isNotEmpty) {
      ...
      _activeTournaments.removeWhere(
          (t) => completedOrCancelledTournamentsThisWeek.contains(t));
      print(
          "Moved ${completedOrCancelledTournamentsThisWeek.length} tournaments to history/cancelled.");
    }
  ...
  void _updateLeagueStandings(Tournament tournament, Match match) {
    ...
    if (homeStanding == null || awayStanding == null) {
      print(
          "Error: Could not find league standings for teams in match ${match.id}");
      return;
    }
  ...
  void _handleTournamentCompletion(Tournament tournament) {
    if (tournament.status != TournamentStatus.Completed)
      return; // Only handle completed, not cancelled

    print("Handling completion of tournament: ${tournament.name}");
    String? determinedWinnerId =
        tournament.winnerId; // Winner might be set by knockout logic
    ...
    if (tournament.format == TournamentFormat.Knockout) {
      if (determinedWinnerId == null && tournament.matches.isNotEmpty) {
        ...
        List<Match> simulatedMatches = tournament.matches
            .where((Match m) => m.isSimulated)
            .toList(); // Explicit type
        if (simulatedMatches.isNotEmpty) {
          simulatedMatches.sort((a, b) =>
              b.matchDate.compareTo(a.matchDate)); // Sort by date descending
          // The winner of the most recent match should be the tournament winner
          determinedWinnerId = simulatedMatches.first.winnerId;
          print(
              " -> Determined knockout winner from last match: $determinedWinnerId");
        } else {
          print(
              " -> Warning: Knockout tournament completed but no simulated matches found to determine winner.");
        }
      }
    }
    // Determine League Winner
      ...
      if (determinedWinnerId == null && tournament.leagueStandings.isNotEmpty) {
        ...
        List<LeagueStanding> sortedStandings =
            tournament.leagueStandings.values.toList();
    ...
    if (determinedWinnerId != null) {
      if (determinedWinnerId == playerAcademyId) {
        ...
        _academyReputation += (tournament.format == TournamentFormat.League)
            ? 60
            : 30; // Increased rep gain
        // --- End Increased Academy Reputation ---
        _addNewsItem(NewsItem.create(
            title: "Tournament Won!",
            description:
                "We won the ${tournament.name} and received ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(prizeMoney)}!",
            type: NewsItemType.MatchResult, // Corrected type
            date: _timeService.currentDate));
        print(
            "Player won ${tournament.name}. Prize: $prizeMoney. New Balance: ${_financeService.balance}. New Rep: $_academyReputation");
      } else {
        RivalAcademy? rivalWinner = _rivalAcademyMap[determinedWinnerId];
        if (rivalWinner != null) {
          winnerName = rivalWinner.name;
          rivalWinner.balance += prizeMoney;
          rivalWinner.reputation +=
              (tournament.format == TournamentFormat.League)
                  ? 40
                  : 20; // Rival rep boost
          _addNewsItem(NewsItem.create(
              ...
              date: _timeService.currentDate));
          print(
              "${rivalWinner.name} won ${tournament.name}. Prize: $prizeMoney. New Balance: ${rivalWinner.balance}. New Rep: ${rivalWinner.reputation}");
        } else {
          ...
          if (aiWinner != null) {
            ...
            _addNewsItem(NewsItem.create(
                ...
                date: _timeService.currentDate));
            print(
                "${aiWinner.name} won ${tournament.name}. Prize: $prizeMoney. New Balance: ${aiWinner.balance}. New Rep: ${aiWinner.reputation}");
          } else {
            winnerName =
                "An unknown team"; // Fallback if neither rival nor AI club found
            print(
                "Warning: Winner ID $determinedWinnerId not found in rival or AI club map for tournament ${tournament.name}");
          }
        }
      }
    } else {
      _addNewsItem(NewsItem.create(
          title: "${tournament.name} Concluded",
          description:
              "The ${tournament.name} has finished.", // Simpler message if no winner determined
          type: NewsItemType.MatchResult, // Corrected type
          date: _timeService.currentDate));
    ...
    if (tournament.format == TournamentFormat.League &&
        tournament.isAIClubFocusedLeague) {
      _handleLeaguePromotionRelegation(tournament);
    }
    // --- END NEW ---
  }

  void _updatePlayerStatsAndFatigue(
      Match match, List<Player> homeLineup, List<Player> awayLineup) {
    ...
    if (match.homeTeamId == playerAcademyId)
      playersToCheck.addAll(
          _academyPlayers.where((p) => match.homeLineup.contains(p.id)));
    if (match.awayTeamId == playerAcademyId)
      playersToCheck.addAll(
          _academyPlayers.where((p) => match.awayLineup.contains(p.id)));
    ...
    for (var academyPlayer in playersToCheck) {
      ...
      double fatigueIncrease = 15.0 +
          ((100 - academyPlayer.stamina) /
              10.0); // Higher increase for lower stamina
      academyPlayer.fatigue =
          (academyPlayer.fatigue + fatigueIncrease).clamp(0.0, 100.0);
  ...
  void _updateRivalOrAIClubFatigue(
      Match match, List<Player> homeLineup, List<Player> awayLineup) {
    if (!match.isSimulated) return;
    ...
    if (homeRival != null) {
      homePlayersToUpdate.addAll(
          homeRival.players.where((p) => match.homeLineup.contains(p.id)));
    } else if (homeAIClub != null) {
      homePlayersToUpdate.addAll(
          homeAIClub.players.where((p) => match.homeLineup.contains(p.id)));
    }
    ...
    if (awayRival != null) {
      awayPlayersToUpdate.addAll(
          awayRival.players.where((p) => match.awayLineup.contains(p.id)));
    } else if (awayAIClub != null) {
      awayPlayersToUpdate.addAll(
          awayAIClub.players.where((p) => match.awayLineup.contains(p.id)));
    }

    // Apply fatigue increase
    List<Player> allPlayersToUpdate = [
      ...homePlayersToUpdate,
      ...awayPlayersToUpdate
    ];
  ...
  void _applyWeeklyFatigueRecovery() {
    ...
    for (var player in _academyPlayers) {
      double recoveryAmount =
          5.0 + (player.stamina / 10.0); // Better recovery for higher stamina
      // Bonus recovery if medical bay is high
      recoveryAmount *=
          (1.0 + (_medicalBayLevel - 1) * 0.1); // +10% per level above 1
  ...
  void _handlePlayerTraining() {
    ...
    final coaches =
        _hiredStaff.where((s) => s.role == StaffRole.Coach).toList();
    if (coaches.isEmpty) {
      /* print("No coaches hired. No specific player training applied this week."); */ return;
    } // Less verbose
    ...
    for (var coach in coaches) {
      ...
      for (var playerId in coach.assignedPlayerIds) {
        if (playerMap.containsKey(playerId) &&
            !trainedPlayerIds.contains(playerId)) {
          ...
          if (player.currentSkill < player.potentialSkill) {
            ...
            switch (_difficulty) {
              ...
              case Difficulty.Hardcore:
                ...
                break; // Added Hardcore penalty
            }
            int totalChance =
                (baseChance + coachBonus + facilityBonus + difficultyModifier)
                    .clamp(1, 99);
            if (_random.nextInt(100) < totalChance) {
              int oldSkill = player.currentSkill;
              bool improved = player.train(
                  focusPosition: player.assignedPosition); // Use train method
              if (improved) {
                ...
                _addNewsItem(NewsItem.create(
                    ...
                    description:
                        "${player.name} showed improvement in training under Coach ${coach.name}. Skill in ${player.assignedPosition.name} is now ${player.currentSkill} (was $oldSkill).",
                    type: NewsItemType.Training,
                    date: _timeService.currentDate));
          ...
          print(
              "  -> Warning: Player ID $playerId assigned to coach ${coach.name} not found in academy players.");
        }
      }
    }
  ...
  void _refreshAvailableStaff() {
    ...
    _availableStaff.removeWhere((staff) {
      bool leaving =
          _random.nextDouble() < 0.20; // 20% chance any staff leaves the market
  ...
  bool assignPlayerToCoach(String playerId, String coachId) {
    Staff? coach = _hiredStaff
        .firstWhereOrNull((s) => s.id == coachId && s.role == StaffRole.Coach);
    if (coach == null) {
    ...
    if (coach.assignedPlayerIds.length >= coach.maxPlayersTrainable) {
      print(
          "Error: Coach ${coach.name} is already at maximum capacity (${coach.maxPlayersTrainable}). Cannot assign ${player.name}.");
      return false;
    }
    if (coach.assignedPlayerIds.contains(playerId)) {
      print(
          "Info: Player ${player.name} is already assigned to coach ${coach.name}.");
      return true;
    }
    unassignPlayerFromAnyCoach(
        playerId); // Ensure player is removed from any other coach first
  ...
  bool unassignPlayerFromCoach(String playerId, String coachId) {
    ...
    if (removed) {
      ...
      print(
          "Info: Player $playerName was not assigned to coach ${coach.name}.");
    }
    return removed;
  }
  ...
  Staff? getCoachForPlayer(String playerId) {
    return _hiredStaff.firstWhereOrNull((s) =>
        s.role == StaffRole.Coach && s.assignedPlayerIds.contains(playerId));
  }
  ...
  int _getTeamSkill(String teamId, List<Player> selectedLineup) {
    ...
    for (var player in selectedLineup) {
      double fatigueModifier = 1.0 -
          (player.fatigue / 200.0); // Fatigue reduces effectiveness (up to 50%)
      totalEffectiveSkill += player.currentSkill * fatigueModifier;
    }
    int averageEffectiveSkill =
        (totalEffectiveSkill / selectedLineup.length).round();

    // Apply Manager Bonus for Player Academy
    if (teamId == playerAcademyId) {
      Staff? manager =
          _hiredStaff.firstWhereOrNull((s) => s.role == StaffRole.Manager);
      if (manager != null) {
        int managerBonus =
            (manager.skill / 20).floor(); // Example: Skill 60 -> +3 bonus
    ...
    // Apply Base Skill Influence for Rival Academies
      ...
      averageEffectiveSkill =
          ((averageEffectiveSkill * 0.7) + (baseRivalSkill * 0.3)).round();
    }
  ...
  int _getBaseRivalAcademySkill(String teamId) {
    ...
    if (academy != null) {
      ...
      switch (_difficulty) {
        case Difficulty.Easy:
          baseSkill = (baseSkill * 0.85).round().clamp(1, 100);
          break;
        case Difficulty.Normal:
          break; // No change
        case Difficulty.Hard:
          baseSkill = (baseSkill * 1.15).round().clamp(1, 100);
          break;
        case Difficulty.Hardcore:
          baseSkill = (baseSkill * 1.25).round().clamp(1, 100);
          break; // Added Hardcore boost
      }
      return baseSkill;
    } else {
      print(
          "Warning: Rival Academy with ID '$teamId' not found in _rivalAcademyMap. Returning default skill.");
  ...
  MatchTeamSelection selectPlayerTeamForMatch(TournamentType type) {
    ...
    Formation formation = manager?.preferredFormation ??
        (predefinedFormations.firstWhereOrNull(
                (f) => f.tournamentType == type) ?? // Find first matching type
    ...
    if (_academyPlayers.length < totalPlayersNeeded) {
      print(
          "Warning: Not enough players in academy (${_academyPlayers.length}) for a ${type.toString()} match (needs $totalPlayersNeeded). Selecting all available.");
  ...
  MatchTeamSelection selectRivalTeamForMatch(
      TournamentType type, RivalAcademy academy) {
    int startersNeeded = _getPlayersNeededForType(type);
    int benchSize = _getBenchSizeForType(type);
    int totalPlayersNeeded = startersNeeded + benchSize;
    ...
    // --- FIX: Use predefinedFormations ---
    Formation formation = predefinedFormations.firstWhereOrNull(
            (f) => f.tournamentType == type) ?? // Find first matching type
        predefinedFormations.first; // Absolute fallback
    ...
    if (academy.players.length < totalPlayersNeeded) {
      print(
          "Warning: Not enough players in rival academy ${academy.name} (${academy.players.length}) for a ${type.toString()} match (needs $totalPlayersNeeded). Selecting all available.");
  ...
  MatchTeamSelection selectAIClubTeamForMatch(
      TournamentType type, AIClub club) {
    int startersNeeded = _getPlayersNeededForType(type);
    int benchSize = _getBenchSizeForType(type);
    int totalPlayersNeeded = startersNeeded + benchSize;

    // AI Clubs use a default formation for now
    Formation formation = predefinedFormations
            .firstWhereOrNull((f) => f.tournamentType == type) ??
        predefinedFormations.first;

    // Check if enough players are available
    if (club.players.length < totalPlayersNeeded) {
      print(
          "Warning: Not enough players in AI club ${club.name} (${club.players.length}) for a ${type.toString()} match (needs $totalPlayersNeeded). Selecting all available.");
  ...
  int _sortByEffectiveSkill(Player a, Player b) {
    double fatiguePenaltyA =
        a.fatigue > 75 ? 50 : (a.fatigue / 2); // Heavier penalty above 75%
  ...
  int _getBenchSizeForType(TournamentType type) {
    switch (type) {
      case TournamentType.threeVthree:
        return 2; // e.g., 3 starters + 2 bench
      case TournamentType.fiveVfive:
        return 3; // e.g., 5 starters + 3 bench
      case TournamentType.sevenVseven:
        return 4; // e.g., 7 starters + 4 bench
      case TournamentType.elevenVeleven:
        return 7; // e.g., 11 starters + 7 bench
      default:
        return 7;
    }
  }
  ...
  void _updateReputationAfterMatch(Tournament tournament, Match match) {
    ...
    bool playerInvolved = match.homeTeamId == playerAcademyId ||
        match.awayTeamId == playerAcademyId;
    int playerReputationChange = 0;
    int playerIndividualRepChangeBase = 0; // Base change for players involved

    if (playerInvolved) {
      bool playerWon = (match.homeTeamId == playerAcademyId &&
      ...
      int winRep = (tournament.format == TournamentFormat.League)
          ? 5
          : 8; // Increased win rep
      int drawRep = (tournament.format == TournamentFormat.League)
          ? 2
          : 3; // Increased draw rep
      int lossRep = (tournament.format == TournamentFormat.League)
          ? -1
          : -3; // Loss rep remains
      // --- End Increased Academy Reputation ---
      ...
      if (playerWon) {
        ...
        playerIndividualRepChangeBase =
            0; // No base rep change for losing, but can gain from goals/assists
      }
      ...
      if (playerWon) {
        ...
        fanChangeOnMatch = (5 + (match.viewership / 20))
            .round(); // e.g., 5 + (100 viewers / 20) = 10 fans
        fanChangeOnMatch = fanChangeOnMatch.clamp(1, 50); // Clamp fan gain
      } else if (playerDrew) {
        // Smaller gain for a draw, scaled by viewership
        fanChangeOnMatch = (2 + (match.viewership / 50))
            .round(); // e.g., 2 + (100 viewers / 50) = 4 fans
      ...
      fanChangeOnMatch += (match.viewership / 100)
          .round()
          .clamp(0, 5); // Max +5 from raw viewership
    ...
    // *** FIX: Use match.homeLineup and match.awayLineup which store the actual player IDs used ***
    if (match.homeTeamId == playerAcademyId)
      participatingPlayerIds.addAll(match.homeLineup);
    if (match.awayTeamId == playerAcademyId)
      participatingPlayerIds.addAll(match.awayLineup);

    for (String playerId in participatingPlayerIds) {
      Player? player =
          _academyPlayers.firstWhereOrNull((p) => p.id == playerId);
      if (player != null) {
    ...
    dynamic homeEntity =
        _rivalAcademyMap[match.homeTeamId] ?? _aiClubMap[match.homeTeamId];
    dynamic awayEntity =
        _rivalAcademyMap[match.awayTeamId] ?? _aiClubMap[match.awayTeamId];

    // Define reputation changes (slightly different for AI clubs vs Rivals)
    int winRep =
        (tournament.format == TournamentFormat.League) ? 2 : 3; // Base win rep
    ...
    if (homeEntity != null) {
      ...
      int entityWinRep =
          winRep + (isAI ? 1 : 0); // AI clubs get slightly more for winning
      int entityDrawRep = drawRep;
      int entityLossRep =
          lossRep - (isAI ? 1 : 0); // AI clubs lose slightly more

      if (match.result == MatchResult.homeWin)
        repChange = entityWinRep;
      else if (match.result == MatchResult.draw)
        repChange = entityDrawRep;
      else
        repChange = entityLossRep; // Lost

      homeEntity.reputation = max(isAI ? 20 : 10,
          currentRep + repChange); // Higher min rep for AI clubs
  ...
  void _generateTransferOffers() {
    ...
    _transferOffers.removeWhere((o) =>
        ...
        o['offeringClubId'] != null &&
        _aiClubMap.containsKey(o['offeringClubId']));
    ...
    if (_difficulty == Difficulty.Hardcore) {
      print(
          "--- DEBUG: Skipping transfer offers due to Hardcore difficulty. ---");
      return; // Skip offer generation in Hardcore for simplicity initially
    }

    final random = Random();
    print(
        "--- DEBUG: Iterating through ${_academyPlayers.length} academy players for offers ---");
    for (var player in _academyPlayers) {
      print(
          "--- DEBUG: Considering player ${player.name} (ID: ${player.id}) ---");
      // --- NEW Offer Chance Calculation ---
      double baseChance = 0.01; // Small base chance
      // Skill Factor (higher skill = much higher chance)
      double skillFactor = pow(player.currentSkill / 100.0, 2) *
          0.45; // Exponential based on skill, up to 0.45
      // Potential Factor (higher potential = higher chance)
      double potentialFactor = pow(player.potentialSkill / 100.0, 2) *
          0.23; // Exponential based on potential, up to 0.23
      // Age Factor (huge bonus for young players 15-18)
      double ageFactor = 0.0;
      if (player.age <= 18) {
        ageFactor =
            (19 - player.age) * 0.05; // Up to 0.6 bonus for 15yo (19-15)*0.05
      ...
      double repMultiplier =
          (player.reputation / 500.0) * (1.0 + _academyReputation / 1000.0);
      repMultiplier = repMultiplier.clamp(0.0, 1.5); // Clamp multiplier effect

      // Combine factors
      double combinedChance =
          (baseChance + skillFactor + potentialFactor + ageFactor) *
              repMultiplier;
      // Clamp final chance (e.g., max 50% per week for exceptional talents)
      double offerChance =
          combinedChance.clamp(0.005, 0.50); // Min 0.5%, Max 50%

      // --- END NEW Offer Chance Calculation ---

      if (random.nextDouble() < offerChance) {
        int marketValue = player
            .calculateMarketValue(); // Calculate value needed for filtering/weighting
        ...
        for (var club in potentialBidders) {
          ...
          double tierWeight =
              (4 - club.tier) * 50.0; // Tier 1=150, Tier 2=100, Tier 3=50
          double repWeight = club.reputation * 0.5;
          double balanceWeight = (club.balance / 1000000.0).clamp(0.1, 1.0) *
              50.0; // Up to 50 bonus based on balance
        ...
        double offerMultiplier = 0.75 +
            random.nextDouble() *
                0.6; // Offer between 75% and 135% of market value
        int offerAmount = (marketValue * offerMultiplier).round();
        ...
        if (offeringClub.balance >= offerAmount) {
          _transferOffers.add({
            ...
            'offerAmount': offerAmount,
            'isAIClubOffer':
                true, // Flag to distinguish from potential future rival offers
            'sellingClubId':
                playerAcademyId, // Ensure selling club is player's academy
            'sellingClubName': _academyName, // Add selling club name
            'dateEpoch': _timeService.currentDate
                .millisecondsSinceEpoch, // Add date epoch for consistency
          });
          ...
          _addNewsItem(NewsItem.create(
              title: "Transfer Offer Received",
              description:
                  "${offeringClub.name} has made an offer of \$${NumberFormat.compact().format(offerAmount)} for ${player.name}.",
              type: NewsItemType.TransferOffer,
              date: _timeService.currentDate));
        } else {
    ...
    print(
        "--- DEBUG: Finished _generateTransferOffers. Found ${_transferOffers.length} offers. ---");
  }
  ...
  void acceptTransferOffer(Map<String, dynamic> offer) {
    ...
    bool isAIClubOffer =
        offer['isAIClubOffer'] ?? false; // Check if it's an AI club

    Player? player = _academyPlayers.firstWhereOrNull((p) => p.id == playerId);

    if (player == null) {
      print("Error accepting transfer: Player $playerId not found.");
      _transferOffers.removeWhere(
          (o) => o['playerId'] == playerId); // Remove invalid offer
      notifyListeners();
      return;
    }
    ...
    if (buyingEntity == null) {
      print(
          "Error accepting transfer: Buying entity $offeringClubId not found (isAIClubOffer: $isAIClubOffer).");
      _transferOffers.removeWhere(
          (o) => o['playerId'] == playerId); // Remove invalid offer
      notifyListeners();
      return;
    ...
    if (canAfford) {
      ...
      _transferOffers
          .removeWhere((o) => o['playerId'] == playerId); // Remove this offer
      _calculateWeeklyWages(); // Recalculate player wages

      print(
          "Accepted transfer offer for ${player.name}. Received $offerAmount. Balance: ${_financeService.balance}");
      print(
          " -> $buyerName signed ${player.name}. New Balance: ${buyingEntity.balance}. Player Count: ${buyingEntity.players.length}");

      _addNewsItem(NewsItem.create(
          title: "Transfer Accepted",
          description:
              "We accepted the offer of \$${NumberFormat.compact().format(offerAmount)} for ${player.name} from $buyerName.",
          type: NewsItemType.TransferDecision,
          date: _timeService.currentDate));
      notifyListeners();
    } else {
      print(
          "Error: $buyerName can no longer afford the offer of $offerAmount for ${player.name}. Rejecting offer.");
  ...
  int getTrainingFacilityUpgradeCost() =>
      _calculateFacilityUpgradeCost(_trainingFacilityLevel);
  int getScoutingFacilityUpgradeCost() =>
      _calculateFacilityUpgradeCost(_scoutingFacilityLevel);
  int getMedicalBayUpgradeCost() =>
      _calculateFacilityUpgradeCost(_medicalBayLevel);
  int getMerchandiseStoreUpgradeCost() => _calculateFacilityUpgradeCost(
      _merchandiseStoreLevel); // Cost for merch store
  ...
  void _updateStaffCapsFromFacilities() {
    ...
    _maxStoreManagers = (_merchandiseStoreLevel > 0)
        ? _merchandiseStoreLevel
        : 0; // 1 Store Manager per store level
  ...
  bool upgradeTrainingFacility() {
    ...
    if (_financeService.canAfford(cost.toDouble())) {
      ...
      _updateStaffCapsFromFacilities();
      print(
          "Upgraded Training Facility to Level $_trainingFacilityLevel. Cost: $cost. Balance: ${_financeService.balance}. Rep: $_academyReputation");
      _addNewsItem(NewsItem.create(
          title: "Facility Upgraded",
          description:
              "Training Facility upgraded to Level $_trainingFacilityLevel. Coach capacity increased to $_maxCoaches. Academy reputation increased.",
      ...
      print(
          "Cannot upgrade Training Facility. Cost: $cost, Balance: ${_financeService.balance}");
      return false;
    }
  }
  ...
  bool upgradeScoutingFacility() {
    ...
    if (_financeService.canAfford(cost.toDouble())) {
      ...
      _updateStaffCapsFromFacilities();
      print(
          "Upgraded Scouting Facility to Level $_scoutingFacilityLevel. Cost: $cost. Balance: ${_financeService.balance}");
      _addNewsItem(NewsItem.create(
          title: "Facility Upgraded",
          description:
              "Scouting Facility upgraded to Level $_scoutingFacilityLevel. Scout capacity increased to $_maxScouts.",
          type: NewsItemType.Facility,
          date: _timeService.currentDate));
      notifyListeners();
      return true;
      ...
      print(
          "Cannot upgrade Scouting Facility. Cost: $cost, Balance: ${_financeService.balance}");
      return false;
    }
  }
  ...
  bool upgradeMedicalBay() {
    ...
    if (_financeService.canAfford(cost.toDouble())) {
      ...
      _updateStaffCapsFromFacilities();
      print(
          "Upgraded Medical Bay to Level $_medicalBayLevel. Cost: $cost. Balance: ${_financeService.balance}");
      _addNewsItem(NewsItem.create(
          title: "Facility Upgraded",
          description:
              "Medical Bay upgraded to Level $_medicalBayLevel. Physio capacity increased to $_maxPhysios.",
          type: NewsItemType.Facility,
          date: _timeService.currentDate));
      notifyListeners();
      return true;
      ...
      print(
          "Cannot upgrade Medical Bay. Cost: $cost, Balance: ${_financeService.balance}");
      return false;
    }
  }
  ...
  bool upgradeMerchandiseStore() {
    ...
    if (_financeService.canAfford(cost.toDouble())) {
      ...
      print(
          "Upgraded Merchandise Store to Level $_merchandiseStoreLevel. Cost: $cost. Balance: ${_financeService.balance}. Rep: $_academyReputation. Max Store Managers: $_maxStoreManagers");
      _addNewsItem(NewsItem.create(
          title: "Facility Upgraded",
          description:
              "Merchandise Store upgraded to Level $_merchandiseStoreLevel. Store Manager capacity increased to $_maxStoreManagers. Academy reputation increased.",
          type: NewsItemType.Facility,
          date: _timeService.currentDate));
      notifyListeners();
      return true;
      ...
      print(
          "Cannot upgrade Merchandise Store. Cost: $cost, Balance: ${_financeService.balance}");
      return false;
    }
  }
  ...
  void _addNewsItem(NewsItem item) {
    _newsItems.insert(0, item); // Insert at beginning (Newest First)
    if (_newsItems.length > 100) {
      // Limit news items
      _newsItems.removeLast(); // Remove oldest (at end)
    }
  ...
  Future<bool> saveGame() async {
    try {
      ...
      if (kIsWeb && e is UnsupportedError) {
        print(
            "--- NOTE: File system operations are not supported on web. Using SharedPreferences. ---");
      }
      return false;
    }
  }
  ...
  Future<bool> loadGame() async {
    try {
      ...
      if (!jsonMap.containsKey('rivalAcademies')) {
        print(
            "--- Save file is from an older version (missing rivalAcademies). Loading might be incomplete or fail. ---");
      ...
      if (jsonMap.containsKey('activeTournaments')) {
        ...
        for (var tJson in activeT) {
          if (tJson is Map<String, dynamic> &&
              !tJson.containsKey('leagueStandings')) {
            tJson['leagueStandings'] = {}; // Add empty map if missing
          }
          if (tJson is Map<String, dynamic> &&
              !tJson.containsKey('currentByeTeamId')) {
      ...
      if (!jsonMap.containsKey('aiClubs')) {
        print(
            "--- Save file is from an older version (missing aiClubs). Adding empty list. ---");
      ...
      _merchandiseStoreLevel = loadedState.merchandiseStoreLevel ??
          0; // Load merchandise store level, default to 0 if not present
      _fans =
          loadedState.fans ?? 100; // Load fans, default to 100 if not present
      _academyReputation = loadedState.academyReputation;
      _newsItems = loadedState.newsItems;
      _newsItems.sort(
          (a, b) => b.date.compareTo(a.date)); // Ensure Newest First order
      ...
      // Catch stacktrace too
      print("--- ERROR LOADING GAME STATE: $e ---");
      print(
          "--- Stacktrace: $stacktrace ---"); // Print stacktrace for debugging
      if (kIsWeb && e is UnsupportedError) {
        print(
            "--- NOTE: File system operations are not supported on web. Using SharedPreferences. ---");
      }
  ...
  void _handleLeaguePromotionRelegation(Tournament league) {
    ...
    if (currentTier == 0) {
      print(
          " -> Cannot determine tier for league ${league.name}. Skipping P/R.");
      return;
    }

    List<LeagueStanding> sortedStandings =
        league.leagueStandings.values.toList();
    if (sortedStandings.length < 4) {
      // Need at least 4 teams for P/R
      print(
          " -> Not enough teams (${sortedStandings.length}) in league for promotion/relegation. Skipping.");
      return;
    }
    ...
    // --- Promotion ---
    if (currentTier > 1) {
      // Can only promote from Tier 2 or 3
      int targetTier = currentTier - 1;
      List<LeagueStanding> promotedTeams =
          sortedStandings.take(numPromote).toList();
      print(
          " -> Promoting to Tier $targetTier: ${promotedTeams.map((s) => s.teamId).join(', ')}");
      for (var standing in promotedTeams) {
        AIClub? club = _aiClubMap[standing.teamId];
        if (club != null) {
          if (club.tier == currentTier) {
            // Only promote if they are currently in this tier
            club.tier = targetTier;
            club.reputation += 25; // Reputation boost for promotion
            print(
                "   -> ${club.name} promoted to Tier $targetTier. New Rep: ${club.reputation}");
            _addNewsItem(NewsItem.create(
                title: "Club Promoted!",
                description:
                    "${club.name} has been promoted to Tier $targetTier after finishing ${sortedStandings.indexOf(standing) + 1}${_getOrdinalSuffix(sortedStandings.indexOf(standing) + 1)} in ${league.name}!",
                type: NewsItemType.LeagueUpdate,
                date: _timeService.currentDate));
          } else {
            print(
                "   -> ${club.name} finished in promotion spot but is already in Tier ${club.tier}. No change.");
          }
          ...
          if (_playerAcademyTier == currentTier) {
            ...
            print(
                "   -> Player Academy promoted to Tier $targetTier! New Rep: $_academyReputation");
            _addNewsItem(NewsItem.create(
                title: "PROMOTED!",
                description:
                    "Your academy has been promoted to Tier $targetTier after finishing ${sortedStandings.indexOf(standing) + 1}${_getOrdinalSuffix(sortedStandings.indexOf(standing) + 1)} in ${league.name}!",
                type: NewsItemType.LeagueUpdate,
                date: _timeService.currentDate));
          } else {
            print(
                "   -> Player Academy finished in promotion spot but is already in Tier $_playerAcademyTier. No change.");
          }
        } else {
          ...
          if (academy != null) {
            if (academy.tier == currentTier) {
              ...
              print(
                  "   -> Rival Academy ${academy.name} promoted to Tier $targetTier. New Rep: ${academy.reputation}");
              _addNewsItem(NewsItem.create(
                  title: "Rival Promoted",
                  description:
                      "${academy.name} has been promoted to Tier $targetTier after finishing ${sortedStandings.indexOf(standing) + 1}${_getOrdinalSuffix(sortedStandings.indexOf(standing) + 1)} in ${league.name}.",
                  type: NewsItemType.LeagueUpdate,
                  date: _timeService.currentDate));
            } else {
              print(
                  "   -> Rival Academy ${academy.name} finished in promotion spot but is already in Tier ${academy.tier}. No change.");
            }
          }
        }
      }
    ...
    // --- Relegation ---
    if (currentTier < 3) {
      // Can only relegate from Tier 1 or 2
      int targetTier = currentTier + 1;
      List<LeagueStanding> relegatedTeams =
          sortedStandings.reversed.take(numRelegate).toList();
      print(
          " -> Relegating to Tier $targetTier: ${relegatedTeams.map((s) => s.teamId).join(', ')}");
      for (var standing in relegatedTeams) {
        AIClub? club = _aiClubMap[standing.teamId];
        if (club != null) {
          if (club.tier == currentTier) {
            // Only relegate if they are currently in this tier
            club.tier = targetTier;
            club.reputation =
                max(10, club.reputation - 15); // Reputation hit for relegation
            print(
                "   -> ${club.name} relegated to Tier $targetTier. New Rep: ${club.reputation}");
            _addNewsItem(NewsItem.create(
                title: "Club Relegated",
                description:
                    "${club.name} has been relegated to Tier $targetTier after finishing ${sortedStandings.indexOf(standing) + 1}${_getOrdinalSuffix(sortedStandings.indexOf(standing) + 1)} in ${league.name}.",
                type: NewsItemType.LeagueUpdate,
                date: _timeService.currentDate));
          } else {
            print(
                "   -> ${club.name} finished in relegation spot but is already in Tier ${club.tier}. No change.");
          }
        } else if (standing.teamId == playerAcademyId) {
          // Handle Player Academy Relegation
          if (_playerAcademyTier == currentTier) {
            _playerAcademyTier = targetTier;
            _academyReputation = max(
                10, _academyReputation - 20); // Player rep hit for relegation
            print(
                "   -> Player Academy relegated to Tier $targetTier. New Rep: $_academyReputation");
            _addNewsItem(NewsItem.create(
                title: "RELEGATED!",
                description:
                    "Your academy has been relegated to Tier $targetTier after finishing ${sortedStandings.indexOf(standing) + 1}${_getOrdinalSuffix(sortedStandings.indexOf(standing) + 1)} in ${league.name}.",
                type: NewsItemType.LeagueUpdate,
                date: _timeService.currentDate));
          } else {
            print(
                "   -> Player Academy finished in relegation spot but is already in Tier $_playerAcademyTier. No change.");
          }
        } else {
          ...
          if (academy != null) {
            if (academy.tier == currentTier) {
              academy.tier = targetTier;
              academy.reputation = max(
                  10, academy.reputation - 15); // Rival rep hit for relegation
              print(
                  "   -> Rival Academy ${academy.name} relegated to Tier $targetTier. New Rep: ${academy.reputation}");
              _addNewsItem(NewsItem.create(
                  title: "Rival Relegated",
                  description:
                      "${academy.name} has been relegated to Tier $targetTier after finishing ${sortedStandings.indexOf(standing) + 1}${_getOrdinalSuffix(sortedStandings.indexOf(standing) + 1)} in ${league.name}.",
                  type: NewsItemType.LeagueUpdate,
                  date: _timeService.currentDate));
            } else {
              print(
                  "   -> Rival Academy ${academy.name} finished in relegation spot but is already in Tier ${academy.tier}. No change.");
            }
          }
        }
      }
  ...
  void _scheduleAnnualLeagues(int year) {
    ...
    DateTime leagueStartDate =
        DateTime(year, 7, 1); // Fixed start date: July 1st
    ...
    proLeagueTemplates
        .sort((a, b) => a.getLeagueTier().compareTo(b.getLeagueTier()));
    ...
    for (var template in proLeagueTemplates) {
      ...
      bool alreadyScheduledOrActive = _activeTournaments.any((t) =>
          ...
          (t.status == TournamentStatus.InProgress ||
              (t.status == TournamentStatus.Scheduled &&
                  t.startDate.year == year))); // Check year for scheduled

      if (alreadyScheduledOrActive) {
        print(
            " -> Instance of ${template.name} for $year already active or scheduled. Skipping.");
        continue;
      }
      ...
      if (leagueTier == 1 || leagueTier == 2) {
        eligibleAIClubs = _aiClubs
            .where((club) =>
                ...
                club.balance >= template.entryFee &&
                club.players.length >= template.requiredPlayers &&
                !club.activeTournamentIds
                    .any((tId) => _activeTournaments.any((at) => at.id == tId)))
            .toList();
      } else {
        // Tier 3 or unknown
        eligibleAIClubs = _aiClubs
      ...
      bool playerEligible = false;
      int youthRepReq =
          template.youthAcademyMinReputation ?? template.requiredReputation;
      ...
      for (var academy in eligibleYouthAcademies) {
        if (participants.length >= slotsToFill) break;
        if (academy is RivalAcademy) {
          // Only add Rivals, not the player placeholder
          participants.add(academy.id);
        }
      }
      ...
      if (participants.length >= minRequired) {
        ...
        print(
            " -> Not enough participants (${participants.length}) for AI Focused League ${template.name}. Min required (allowing for player): $minRequired. League will not be scheduled this year.");
      }
      ...
      if (enoughParticipantsFound) {
        ...
        String joinWindow =
            "You have until ${DateFormat.yMMMd().format(leagueStartDate)} to join.";
        _addNewsItem(NewsItem.create(
            title: "Pro League Scheduled",
            description:
                "${newTournament.name} is scheduled to start on ${DateFormat.yMMMd().format(newTournament.startDate)} with ${participants.length} teams confirmed so far. $joinWindow",
            type: NewsItemType.Tournament,
  ...
  void _scheduleInitialProLeagues() {
    ...
    for (var template in proLeagueTemplates) {
      ...
      print(
          " -> Attempting to schedule initial ${template.name} for $initialYear season.");
      List<String> participants = [];
      bool enoughParticipantsFound = false;
      ...
      print(
          " -> Found ${eligibleAIClubs.length} eligible AI Clubs and ${eligibleYouthAcademies.length} eligible Youth Academies (Player eligible: $playerEligible) for Initial Tier $leagueTier League.");
      ...
      for (var academy in eligibleYouthAcademies) {
        if (participants.length >= slotsToFill) break;
        if (academy is RivalAcademy) participants.add(academy.id);
      }
      print(" -> Added ${participants.length - aiClubsAdded} Youth Academies.");
      ...
      if (participants.length >= minRequired)
        ...
        print(
            " -> Not enough participants (${participants.length}) for Initial League ${template.name}. Min required (allowing for player): $minRequired. League will not be scheduled.");
      // --- End Participant Gathering ---
      ...
      if (enoughParticipantsFound) {
        ...
        Tournament newTournament =
            Tournament.fromTemplate(template, participants, schedulingDate);
        ...
        _addNewsItem(NewsItem.create(
            title: "Pro League Scheduled",
            description:
                "${newTournament.name} is scheduled to start soon with ${participants.length} teams confirmed. $joinWindow",
            type: NewsItemType.Tournament,
            date: _timeService.currentDate));
        print(
            " -> Scheduled Initial ${newTournament.name} (ID: ${newTournament.id}) starting around ${DateFormat.yMMMd().format(newTournament.startDate)} with ${participants.length} teams.");
      }
    }
  ...
  void _handleMerchandiseAndFans() {
    ...
    if (_random.nextDouble() < 0.1) {
      // 10% chance of fan change event
      fanChange = _random.nextInt(11) - 5; // -5 to +5 fans
      // Bonus for high reputation
      if (_academyReputation > 300)
        fanChange += _random.nextInt((_academyReputation ~/ 100));
    ...
    Staff? merchManager = _hiredStaff
        .firstWhereOrNull((s) => s.role == StaffRole.MerchandiseManager);
    ...
    if (merchManager != null && _merchandiseStoreLevel <= 1) {
      ...
      double atGameSales = (_fans * 0.05) *
          (merchManager.skill / 100.0) *
          (0.5 + _random.nextDouble() * 0.5);
      merchandiseIncomeThisWeek +=
          atGameSales.clamp(0, 500); // Cap at-game sales
    }

    // 2. Sales from Merchandise Store (if built and level > 0)
    if (_merchandiseStoreLevel > 0 && merchManager != null) {
      double storeBaseIncome =
          _merchandiseStoreLevel * 100.0; // Base income per store level
      double fanMultiplier = 1 + (_fans / 500.0); // More fans, more sales
      double managerSkillMultiplier =
          0.5 + (merchManager.skill / 200.0); // Skill up to 1.0 multiplier
      double randomFactor = 0.8 + _random.nextDouble() * 0.4; // 0.8 to 1.2

      double storeIncome = storeBaseIncome *
          fanMultiplier *
          managerSkillMultiplier *
          randomFactor;

      // Potential for negative outcome if manager skill is very low or bad luck
      if (merchManager.skill < 30 && _random.nextDouble() < 0.15) {
        // 15% chance of negative if skill < 30
        double lossAmount = storeIncome *
            (0.2 +
                _random.nextDouble() * 0.3); // Lose 20-50% of potential income
        storeIncome -= lossAmount;
        _addNewsItem(NewsItem.create(
            title: "Merchandise Mismanagement",
            description:
                "Poor handling at the club store led to a loss of ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(lossAmount.abs())} this week.",
      ...
      double storeBaseIncome =
          _merchandiseStoreLevel * 50.0; // Reduced base without manager
      double fanMultiplier = 1 + (_fans / 1000.0); // Reduced fan impact
      merchandiseIncomeThisWeek += storeBaseIncome *
          fanMultiplier *
          (0.5 + _random.nextDouble() * 0.3); // Lower random factor

      if (_random.nextDouble() < 0.25) {
        // 25% chance of a small loss
        double lossAmount = (storeBaseIncome * fanMultiplier) *
            (0.1 + _random.nextDouble() * 0.2);
        merchandiseIncomeThisWeek -= lossAmount;
        _addNewsItem(NewsItem.create(
            title: "Merch Store Issues",
            description:
                "The unmanaged club store incurred a small loss of ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(lossAmount.abs())} this week.",
            type: NewsItemType.Finance,
            date: _timeService.currentDate));
      }
    }

    if (merchandiseIncomeThisWeek.abs() > 0.01) {
      // Only add to balance and news if there's actual income/loss
      ...
      String incomeOrLossString =
          merchandiseIncomeThisWeek >= 0 ? "income" : "loss";
      _addNewsItem(NewsItem.create(
          title: "Merchandise Sales Update",
          description:
              "This week's merchandise $incomeOrLossString: ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(merchandiseIncomeThisWeek)}. Current fans: $_fans.",

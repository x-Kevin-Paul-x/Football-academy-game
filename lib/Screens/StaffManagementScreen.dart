...
class StaffManagementScreen extends StatelessWidget {
        ...
        body: Consumer<GameStateManager>(
          builder: (context, gameStateManager, child) {
            ...
            return Column(
              // Wrap with Column
              children: [
                _buildStaffCapsSummary(
                    context, gameStateManager), // Add the summary here
                const Divider(),
                Expanded(
                  // TabBarView needs to be Expanded within a Column
                  child: TabBarView(
                    children: [
                      _buildStaffList(context, hiredStaff, isHiredList: true),
                      _buildStaffList(context, availableStaff,
                          isHiredList: false),
                    ],
                  ),
                ),
  ...
  Widget _buildStaffCapsSummary(
      BuildContext context, GameStateManager gameState) {
    TextStyle capStyle = Theme.of(context).textTheme.bodyMedium!;
    TextStyle countStyle(int current, int max) => capStyle.copyWith(
          color: current >= max
              ? Colors.redAccent
              : (current > 0 ? Colors.green : null),
          fontWeight: current > 0 ? FontWeight.bold : null,
        );

    int hiredManagers =
        gameState.hiredStaff.where((s) => s.role == StaffRole.Manager).length;
    int hiredCoaches =
        gameState.hiredStaff.where((s) => s.role == StaffRole.Coach).length;
    int hiredScouts =
        gameState.hiredStaff.where((s) => s.role == StaffRole.Scout).length;
    int hiredPhysios =
        gameState.hiredStaff.where((s) => s.role == StaffRole.Physio).length;
    int hiredMerchManagers = gameState.hiredStaff
        .where((s) => s.role == StaffRole.MerchandiseManager)
        .length;
    int totalMerchCap =
        gameState.maxStoreManagers + gameState.maxMatchSalesManagers;
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: [
          ...
          Text('$hiredCoaches/${gameState.maxCoaches}',
              style: countStyle(hiredCoaches, gameState.maxCoaches)),
          Text('Scouts: ', style: capStyle),
          Text('$hiredScouts/${gameState.maxScouts}',
              style: countStyle(hiredScouts, gameState.maxScouts)),
          Text('Physios: ', style: capStyle),
          Text('$hiredPhysios/${gameState.maxPhysios}',
              style: countStyle(hiredPhysios, gameState.maxPhysios)),
          Text('Merch Mgrs: ', style: capStyle),
          Tooltip(
            message:
                'Store Mgrs: ${gameState.maxStoreManagers}\nMatch Sales Mgrs: ${gameState.maxMatchSalesManagers}',
            child: Text('$hiredMerchManagers/$totalMerchCap',
                style: countStyle(hiredMerchManagers, totalMerchCap)),
          ),
        ],
      ),
    );
  ...
  Widget _buildStaffList(BuildContext context, List<Staff> staffList,
      {required bool isHiredList}) {
    if (staffList.isEmpty) {
      return Center(
        child: Text(
          isHiredList
              ? 'No staff hired yet.'
              : 'No staff currently available for hire.',
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        ...
        return Consumer<GameStateManager>(
          ...
          try {
            currentCoachState =
                gameStateManager.hiredStaff.firstWhere((s) => s.id == coach.id);
          } catch (e) {
          ...
          final List<Player> assignedToThisCoach =
              academyPlayers.where((p) => assignedIds.contains(p.id)).toList();
          ...
          return AlertDialog(
            title: Text(
                'Assign Players to ${currentCoachState.name} (${assignedIds.length}/$maxCapacity)'),
            content: SizedBox(
              // Use SizedBox to constrain dialog size
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                            ...
                            trailing: IconButton(
                              ...
                              onPressed: () {
                                ...
                                gameStateManager.unassignPlayerFromCoach(
                                    player.id, currentCoachState.id);
                    ...
                    const Text('Available to Assign:',
                        style: TextStyle(fontWeight: FontWeight.bold)),
                    if (unassignedPlayers.isEmpty)
                      ...
                      ...unassignedPlayers.map((player) => ListTile(
                            ...
                            trailing: IconButton(
                              ...
                              onPressed: assignedIds.length >= maxCapacity
                                  ? null
                                  : () {
                                      // Call GameStateManager to assign
                                      bool success =
                                          gameStateManager.assignPlayerToCoach(
                                              player.id, currentCoachState.id);
                                      if (!success && context.mounted) {
                                        ...
                                        ScaffoldMessenger.of(context)
                                          ...
                                          SnackBar(
                                              content: Text(
                                                  'Could not assign ${player.name}. Coach might be full.'),

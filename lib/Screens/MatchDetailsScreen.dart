...
class MatchDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Find the tournament
    final tournament = gameState.activeTournaments
            .firstWhereOrNull((t) => t.id == tournamentId) ??
        gameState.completedTournaments
            .firstWhereOrNull((t) => t.id == tournamentId);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            _buildLineups(
                context, match, gameState, homeTeamName, awayTeamName),
          ],
        ),
      ),
    );
  ...
  Widget _buildMatchInfo(BuildContext context, Match match, String homeTeamName,
      String awayTeamName) {
    return Card(
      elevation: 2,
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (match.isSimulated) Text('Viewership: ${match.viewership}'),
            Text('Round: ${match.round}'),
            if (match.isSimulated)
              Text('Result: ${match.result?.name ?? 'N/A'}'),
            if (!match.isSimulated) const Text('Status: Pending Simulation'),
          ],
        ),
      ),
    );
  ...
  Widget _buildEventLog(
    ...
    final sortedEvents = List<MatchEvent>.from(match.eventLog)
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        String teamName =
            _getTeamName(event.teamId, gameState); // Get team name for context

        // Find player name (check player academy, rivals, then AI clubs)
        Player? player = gameState.academyPlayers
            .firstWhereOrNull((p) => p.id == event.playerId);
        if (player == null) {
          RivalAcademy? rivalAcademy = gameState.rivalAcademyMap[event.teamId];
          player = rivalAcademy?.players
              .firstWhereOrNull((p) => p.id == event.playerId);
        }
        if (player == null) {
          // --- ADDED: Check AI Clubs ---
          AIClub? aiClub = gameState.aiClubMap[event.teamId];
          player =
              aiClub?.players.firstWhereOrNull((p) => p.id == event.playerId);
        } // --- END ADDED ---
        playerName = player?.name ??
            (event.playerId ??
                'Unknown Player'); // Use player ID if name not found

        IconData icon = Icons.info_outline; // Default icon
        Color iconColor = Colors.grey;
        String description =
            event.description; // Use the description from the event by default
        ...
        switch (event.type) {
          ...
          case MatchEventType.Substitution:
            ...
            description =
                'Substitution ($teamName)'; // Description might need more detail from event data
            break;
          ...
          case MatchEventType.PenaltyShootout: // Added case
            ...
            iconColor = event.description.toLowerCase().contains('scored')
                ...
                : Colors.red.shade600;
            description =
                event.description; // Use the full description from the event
            break;
  ...
  Widget _buildLineups(BuildContext context, Match match,
      GameStateManager gameState, String homeTeamName, String awayTeamName) {
    ...
    List<Player> homePlayers =
        _resolvePlayers(match.homeLineup, match.homeTeamId, gameState);
    List<Player> awayPlayers =
        _resolvePlayers(match.awayLineup, match.awayTeamId, gameState);
          ...
          child: Column(
            ...
            children: [
              ...
              if (homePlayers.isNotEmpty)
                ...homePlayers
                    .map((p) => Text('${p.name} (Skill: ${p.currentSkill})'))
                    .toList()
              else
                const Text(
                    'Lineup unavailable'), // Indicate if players couldn't be resolved
            ],
          ),
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              if (awayPlayers.isNotEmpty)
                ...awayPlayers.map((p) => Text(
                    '${p.name} (Skill: ${p.currentSkill})')) // Removed .toList()
              else
                const Text(
                    'Lineup unavailable'), // Indicate if players couldn't be resolved
            ],
  ...
  List<Player> _resolvePlayers(
      List<String> playerIds, String teamId, GameStateManager gameState) {
    List<Player> players = [];
    if (teamId == GameStateManager.playerAcademyId) {
      players = gameState.academyPlayers
          .where((p) => playerIds.contains(p.id))
          .toList();
    } else if (gameState.rivalAcademyMap.containsKey(teamId)) {
      // Check Rivals
      RivalAcademy? academy = gameState.rivalAcademyMap[teamId];
      if (academy != null) {
        players =
            academy.players.where((p) => playerIds.contains(p.id)).toList();
      }
    } else if (gameState.aiClubMap.containsKey(teamId)) {
      // Check AI Clubs
      ...
      print(
          "Warning: Could not resolve players for team ID $teamId in MatchDetailsScreen._resolvePlayers");
    }
